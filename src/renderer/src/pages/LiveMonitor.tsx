    ...
    <div>
      ...
      <div className="card" style={{ padding: 20 }}>
        ...
        <table className="lh">
          ...
          <tbody>
            {sources.length === 0 && (
              <tr><td colSpan={8} style={{ textAlign: 'center', color: '#6b7280', padding: 28 }}>No sources yet. Turn on Live monitoring to seed defaults, or add your own.</td></tr>
            )}
            {sources.map((s) => {
              ...
              return (
                <tr key={s.id}>
                  ...
                  <td style={{ color: '#6b7280', maxWidth: 320, overflow: 'hidden', textOverflow: 'ellipsis' }}>
                    {s.kind === 'google_news' ? cfg.query : s.url}
                  </td>
                  <td style={{ fontSize: 12, color: '#4b5563' }}>{formatInterval(s.poll_interval_seconds)}</td>
